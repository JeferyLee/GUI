...
class Application(Frame):
    ...
    def layout(self):
        # 定义读取数据按钮
        self.btn_rddata=tk.Button(self.root, text='读取数据', command=self.read_data)
        # self.btn_rddata.place(x=500, y=50, width=90, height=50)

        # 定义设置参数按钮
        self.btn_configParam = tk.Button(self.root,text='设置参数',command=self.configParam)
        ...
        self.entry=tk.Entry(self.root, insertbackground='blue', highlightthickness =2)
        entry1=tk.Entry(self.root, insertbackground='blue', highlightthickness =2)
        entry1.pack()
        # 定义画布
        self.cav = tk.Canvas(self.root, width=1920,height=1080, bg='white')
        self.img = tk.PhotoImage(file='3.gif')
        self.imageFile = self.cav.create_image(950, 540, image=self.img)

        self.cav.create_window(500, 500, height=80, width=40, window=entry1)
        # self.cav.place(x=200, y=200)
        self.cav.create_window(100, 150, width=100, height=40,  # 这里的长和宽是定义按钮的长和宽,100和150是按钮的位置
                             window=entry1)
        self.cav.pack()
